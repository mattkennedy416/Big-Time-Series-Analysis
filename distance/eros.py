...
def weight_vector(evals):
    ...
    for j in range(0, len(evals[0])):
        ...
        mean_val = np.mean( [float(evals[i][j]) for i in range(0, len(evals))] )
...
def eros(data):
    ...
    for i in range(0, len(evecs)):
        for j in range(0, len(evecs)):
            ...
            sim_matrix[i, j] = np.sum([float( w[n] * np.abs(np.inner(np.squeeze(np.asarray(evecs[i][:, n])), np.squeeze(np.asarray(evecs[j][:, n]))))) for n in range(0, len(evecs[i]))])
    return (sim_matrix)